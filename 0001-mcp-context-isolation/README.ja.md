...
### 4. MCP を放棄 - Skills + Scripts（直接 API 呼び出し）を使用

MCP の代わりに bash/curl を使用して Skills で API 呼び出しをラップ。
...
| **メンテナンス** | MCP は上流で更新 | 各スクリプトを維持する必要 |
...
MCP の価値はツールだけではありません—**集中的で安全な認証管理**です。コンテキスト隔離はこの利点を維持しながら、コンテキスト消費の問題を解決します。
